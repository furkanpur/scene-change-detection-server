...
class Evaluate:
    ...
    def run(self):
        ...
        self.model.load_state_dict(torch.load(self.fn_model, map_location=torch.device('cpu')))
...
class evaluate_pcd(Evaluate):
    ...
    def run(self):
        ...
        self.fn_model = os.path.abspath("../../../../resource/model_00037320.pth")
        super(evaluate_pcd, self).run()
        f_metrics = open(pjoin(self.resultdir, "eval_metrics(dataset).csv"), 'a+')
        metrics_writer = csv.writer(f_metrics)

        for ds in tqdm(['TSUNAMI']):
