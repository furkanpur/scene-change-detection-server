...
from core.ai.scene_change.model.attention import Temporal_Attention
from core.ai.scene_change.model.util import *
...
class AttentionModule(nn.Module):
    ...
    def __init__(self, local_kernel_size=1, stride=1, padding=0, groups=1,
        ...
        if not drtam:
            self.attention_layer1 = Temporal_Attention(channels[0], channels[0], local_kernel_size, stride, padding,
                                                       groups, refinement=refinement)
            self.attention_layer2 = Temporal_Attention(channels[1], channels[1], local_kernel_size, stride, padding,
                                                       groups, refinement=refinement)
            self.attention_layer3 = Temporal_Attention(channels[2], channels[2], local_kernel_size, stride, padding,
                                                       groups, refinement=refinement)
            self.attention_layer4 = Temporal_Attention(channels[3], channels[3], local_kernel_size, stride, padding,
                                                       groups, refinement=refinement)
        else:
...
def get_attentionmodule(local_kernel_size=1, stride=1, padding=0, groups=1, drtam=False, refinement=False,
                        channels=[64, 128, 256, 512]):
